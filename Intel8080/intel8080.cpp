...
DLL void Stop()
{
	...
	} while (!stepping && cnt++ < 10000000);  // wait to enter stepping state
...
{
	UINT8 v = RAM[addr]; // get the byte at this address
...
DLL void Deposit(UINT16 addr, UINT8 data)
{
	if (stepping) // only if we're in stepping mode
	{
		RAM[addr] = data; // poke the byte into ram
		*PC = addr; // set the PC to this address
		updatedPC = 1;
	}
}
...
DLL int TestDLL()
{
	MessageBox(0, (LPCWSTR)L"DLL Entry OK", (LPCWSTR)L"Test", MB_OK);
	return currentOP;
}
...
BOOL CF() { return *F & 1 ? TRUE : FALSE; }
BOOL PF() { return *F & 4 ? TRUE : FALSE; }
BOOL HF() { return *F & 16 ? TRUE : FALSE; }
BOOL ZF() { return *F & 64 ? TRUE : FALSE; }
BOOL SF() { return *F & 128 ? TRUE : FALSE; }

void CF(BOOL v) { *F &= 254; if (v) *F |= 1; }
void PF(BOOL v) { *F &= 251; if (v) *F |= 4; }
void HF(BOOL v) { *F &= 239; if (v) *F |= 16; }
void ZF(BOOL v) { *F &= 191; if (v) *F |= 64; }
void SF(BOOL v) { *F &= 127; if (v) *F |= 128; }
